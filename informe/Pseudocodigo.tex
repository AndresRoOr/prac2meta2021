	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}
			...
			\IF{$cromosoma.recalcular != true$}
			\STATE $ coste \leftarrow EvaluarSolucion(cromosoma)$
			\STATE $ cromosoma.coste \leftarrow coste$
			\IF{$coste > mejorCoste$}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}
			\FOR{i=0; i$<$numIndividuosPoblacion \& poblacionHijos.tamaño $<$ numIndividuosPoblacion; i+=2}
			...
			\IF{probRepro $<$ probabilidadCruce }
			...
			\STATE $padre2 \leftarrow padre2-\{gen\}$
			\ENDIF
			\ENDFOR
			\FOR{$gen \in padre2$}
			\STATE $hijo \leftarrow hijo\cup gen$
			...
			\IF{poblacionHijos.tamaño $<$ numIndividuosPoblacion }
			\STATE $poblacionHijos \leftarrow poblacionHijos\cup\{padre2\}$
			\ENDIF
			\ENDIF
			\IF{i $==$ numIndividuosPoblacion -2}
			\STATE i $\leftarrow$ 0
			\ENDIF
			\ENDFOR
			\RETURN poblacionHijos
